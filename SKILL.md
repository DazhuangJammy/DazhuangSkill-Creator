...
用这个 skill 来创建、诊断、重写、评估、打包其他 skill。把当前 `SKILL.md` 所在目录视为 `<skill-base>`，所有 bundled resources 都从这里解析，不要依赖调用方当前工作目录。
...
- 新建 skill 和重构 skill 共用同一套架构目标：主 body 扛耐久规则和默认 workflow，只有复杂到会漂移时才加紧凑 `# 索引`，其余长解释 / 模板 / 重复动作再按需要下沉。
...
- `结构重构` 的目标是：把主 body 压回耐久规则 + 默认 workflow，把低频内容下沉到 bundled resources。
